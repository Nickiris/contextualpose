...
def main(final_output_dir):
    args = parse_args()
    cfg = get_cfg()
    cfg.DATASET.TEST = args.test_path
    cfg.TEST.MODEL_FILE  = args.model_path

    logger, _ = setup_logger(final_output_dir, 0, 'test')
    ...
    res_file = os.path.join(res_folder, "keypoints_test-dev2017_results.json")
    json.dump(all_preds, open(res_file, 'w'))


if __name__ == "__main__":
    final_output_dir = "experiment"
    main(final_output_dir)